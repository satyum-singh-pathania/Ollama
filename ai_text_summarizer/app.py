...
def summarize_text(text: str = Form(...)):
    ...
    try:
        ...
        response = requests.post(
            OLLAMA_URL,
            json={"model": "Mistral",
                  "prompt": f"Summarize this : {text}", "stream": False},
            headers=headers
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Invalid JSON response from Ollama API")

        # Extract AI-generated response
        summarized_text = json_response.get(
            "response", "No valid summary received.")
